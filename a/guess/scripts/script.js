...
let currentWord, currentHint, currentDinop, currentDinoimg, correctLetters, wrongGuessCount;
const totalWords = 6;
const maxGuesses = 8;
let word_id = 0
let mustguess = 0;
...
const getNextWord = () => {
    const { word, hint, dinop, dinoimg } = wordList[word_id];
    currentWord = word;
...
const gameOver = (isVictory, nextButton) => {
    setTimeout(() => {
        let modaldinop = ``;
        let modaldinoimg = `img/lost.gif`;
        let modalText2 = ``;
        let imgElement = gameModal.querySelector("img");

        if (!imgElement.classList.contains("lost")) {
            imgElement.classList.add("lost");
        }

        if (isVictory) {
            imgElement.classList.remove("lost");
            modaldinop = `<b>${currentDinop}</b>`;
            modaldinoimg = `${currentDinoimg}`;
            modalText2 = `Βρήκες τη λέξη: <b>${currentWord}</b>`;
        }

        gameModal.querySelector("img").src = modaldinoimg;
        gameModal.querySelector("h4").innerText = `${isVictory ? 'Μπράβο!' : 'Λυπάμαι, δεν βρήκες τη λέξη.'}`;
        gameModal.querySelector(".dino").innerHTML = modaldinop;
        gameModal.querySelector(".found").innerHTML = `${modalText2}`;
